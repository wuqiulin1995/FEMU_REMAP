...
void INIT_STAT_COUNT(struct ssdstate *ssd)
{
	...
	write req between print, avg write delay, avg write size, max write delay, max ref cnt, use nvram fail\n");
...
void stat_print(struct ssdstate *ssd)
{
	...
	fprintf(fout, "%d, %u, %u, %u, %u, %u, %lu, %lu, %lu, %lu, %lu, %lu, %u, %u, %u, %lu, %u, %lu, %u, %lu, %lu, %lu, %d, %u, %u\n", 
		ssd->stat_type, 
		...
		ssd->stat_write_req_print, ssd->stat_avg_write_delay, ssd->stat_avg_req_size, ssd->stat_max_write_delay, ssd->max_valid_array, ssd->stat_use_remap_fail, ssd->stat_gc_remap_fail);
